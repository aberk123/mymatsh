...
export async function POST(_request: Request, { params }: { params: { id: string } }) {
  ...
  const { data: maschil, error: fetchError } = await (adminClient.from('maschils') as any)
    .select('user_id')
    .eq('id', params.id)
    .maybeSingle() as { data: { user_id: string } | null; error: unknown }

  if (fetchError || !maschil) {
    return NextResponse.json({ error: 'Maschil not found' }, { status: 404 })
  }

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error: maschilError } = await (adminClient.from('maschils') as any)
    .update({ is_approved: true })
    .eq('id', params.id)

  if (maschilError) {
    return NextResponse.json({ error: (maschilError as { message: string }).message }, { status: 500 })
  }

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error: userError } = await (adminClient.from('users') as any)
    .update({ status: 'active' })
    .eq('id', maschil.user_id)
