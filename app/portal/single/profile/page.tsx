...
function FieldRow({
  ...
  inputType = 'input', fieldKey = '', editValues = {}, onChange, hint,
}: FieldRowProps) {
...
export default function SingleProfilePage() {
    ...
    try {
      ...
      for (const [k, v] of Object.entries(editValues)) {
        if (k === 'height_inches') {
          ...
          payload.high_schools = v.trim() ? v.split(',').map(s => s.trim()).filter(Boolean) : null
        } else {
          payload[k] = v !== '' ? v : null
        }
      }
      ...
      if (!res.ok) { const json = await res.json(); setSaveError(json.error ?? 'Failed to save.'); return }
      setProfile(p => ({
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!allowed.includes(file.type)) {
      setPhotoError('Only JPG, PNG and WebP files are supported.')
      return
    }
  ...
  const handleCropConfirm = useCallback(async () => {
    ...
    if (!blob) { setPhotoError('Failed to process image. Please try again.'); return }
    ...
    try {
      ...
      // Cache-bust so the browser fetches the fresh image
  ...
  return (
    <AppLayout navItems={navItems} title="My Profile" role="single">
      ...
      <Dialog open={showCropModal} onOpenChange={open => { if (!open) setShowCropModal(false) }}>
        <DialogContent className="max-w-lg w-full sm:max-w-lg max-h-[90vh] flex flex-col">
          <DialogHeader>
          ...
          <p className="text-xs text-center text-[#888888] px-4">Drag to reposition · Resize to adjust the crop area</p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              {photoUrl && (
                <button
                  ...
                  disabled={photoUploading || photoRemoving}
                  className="flex items-center gap-1.5 text-sm text-[#888888] hover:text-red-600 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {photoRemoving
              ...
              <p className="text-xs text-[#AAAAAA] mt-1">JPG, PNG or WebP · Max 5 MB · Square crop enforced</p>
            </div>
          </div>
        ...
        <div className="card">
          ...
          <FieldRow
            ...
            editable inputType="number" fieldKey="height_inches"
        ...
        <div className="card xl:col-span-2">
          <div className="flex items-start justify-between gap-4 mb-3">
            <div>
              ...
              <p className="text-xs text-[#888888] mt-0.5">Labels you select are visible to Shadchanim and help them find you when searching.</p>
            </div>
            {labelsEditMode ? (
            ...
            <div className="flex flex-wrap gap-2">
              {selfLabels.length > 0 ? (
                selfLabels.map(label => (
                  <span key={label} className="text-xs bg-[#F8F0F5] text-brand-maroon px-3 py-1 rounded-full font-medium">{label}</span>
                ))
              ) : (
