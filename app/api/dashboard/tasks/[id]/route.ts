...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: task } = await (adminClient.from('calendar_tasks') as any)
    .select('shadchan_id')
    .eq('id', params.id)
    .maybeSingle() as { data: { shadchan_id: string } | null }

  if (!task) return NextResponse.json({ error: 'Task not found' }, { status: 404 })
  if (task.shadchan_id !== profile.id) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })

  const body = await request.json() as Record<string, unknown>
  const allowed = ['status', 'title', 'type', 'due_date', 'notes']
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error: dbError } = await (adminClient.from('calendar_tasks') as any)
    .update(updates)
    .eq('id', params.id)
