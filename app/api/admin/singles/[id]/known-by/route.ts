...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: rows } = await (admin.from('shadchan_singles') as any)
    ...
    .eq('single_id', singleId) as { data: Array<{ shadchan_id: string; created_at: string }> | null }
  ...
  const { data: profiles } = await (admin.from('shadchan_profiles') as any)
    ...
    .in('id', ids) as { data: Array<{ id: string; full_name: string; city: string | null; state: string | null }> | null }
